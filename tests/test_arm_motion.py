...
def test_left_arm_uses_the_decision_variable_start_time() -> None:
    """The left arm should stay still before `t1` and finish 0.3 s after it starts."""
    ...
    before = motion.left(0.0)
    start = motion.left(0.15)
    final = motion.left(0.45)

    assert motion.left_end == pytest.approx(0.45)
    assert before.elevation.position == pytest.approx(-math.pi)
    assert before.elevation.velocity == pytest.approx(0.0)
    assert start.elevation_plane.position == pytest.approx(math.radians(-20.0))
    assert final.elevation.position == pytest.approx(0.0)
    assert final.elevation_plane.position == pytest.approx(math.radians(10.0))


def test_right_arm_uses_the_fixed_motion_window() -> None:
    """The right arm should always move between 0.0 s and 0.3 s."""
    ...
    initial = motion.right(0.0)
    end = motion.right(0.3)

    assert motion.right_end == pytest.approx(0.3)
    assert initial.elevation.position == pytest.approx(math.pi)
    assert initial.elevation_plane.position == pytest.approx(math.radians(-45.0))
